...
export function ticketsRouter(client: Client, db: DatabaseSync) {
  ...
  router.post('/:guildId/panels/:panelId/categories', requireGuild, async (req, res) => {
    ...
    const guild = client.guilds.cache.get(req.params.guildId);
    if (guild) await _ticketsModule?.rebuildPanelMessage(req.params.panelId, db, guild);
  ...
  router.get('/:guildId', requireGuild, (req, res) => {
    ...
    const rows = status
      ? db.prepare('SELECT * FROM tickets WHERE guild_id = ? AND status = ? ORDER BY created_at DESC LIMIT 100').all(req.params.guildId, String(status))
